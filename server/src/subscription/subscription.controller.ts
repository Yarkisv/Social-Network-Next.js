...
export class SubscriptionController {
  ...
  async findAllById(@Param('id') id: number) {
    ...
    const { subscriptions, subscribers } = await this.subscriptionService.findAllById(id);
  ...
  async checkIsAlreadySubscribed(
    ...
    console.log('Current user: ', current_user_id, ' viewed user: ', viewed_user_id);
  ...
  @Delete('delete/:id')
  @UseGuards(AuthGuard)
  async deleteSubscription(@Param('id') viewed_user_id: number, @Request() req) {
    const id = req.user.user_id;

    console.log('current user: ', id);
    console.log('viewed user: ', viewed_user_id);

    return await this.subscriptionService.deleteSubscription(id, viewed_user_id);
  }
}
